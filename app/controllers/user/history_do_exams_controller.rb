class User::HistoryDoExamsController < ApplicationController
  ...
  def show
    # byebug
    @history_exam.each do |history|
      question = Question.find_by id: history.question_id
      question_content = question.content
      answer_content = question.answers.find_by(is_correct_answer: true, question_id: question.id).content
      answer = history.answer
      # if answer_content === history.answer
      #   history.is_true = true
      # else
      #   history.is_true = false
      # end
    end
  ...
  def load_history
    @history_exam = HistoryDoExam.includes(:user, :question).where(amount_test: params[:amount_test].to_i)
    return if @history_exam
